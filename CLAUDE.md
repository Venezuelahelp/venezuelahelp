...
> **El scrape es asíncrono.** `POST /scrape` (botón "Scrape ahora" del admin) invoca el Lambda del scraper con `InvocationType: Event` y devuelve **202 `{started:true}`** de inmediato; la extracción sigue en segundo plano (**~11 min** con todas las fuentes: baja HTML, extrae, pagina con throttle, llama a Bedrock por cada fuente IA que cambió). **No hay señal de "terminó"**. <!-- /aprende 2026-07-01: el scrape tarda ~11 min, no 1-2; `generatedAt` se sella al INICIO y el `last-modified` de S3 al final → el snapshot vivo refleja el scrape ANTERIOR hasta ~11 min después de :28/:58 --> Por eso el admin muestra un aviso "Scrape iniciado…" al disparar y un botón "Actualizar" en el Dashboard para re-consultar conteos/estado cuando el usuario quiera. No esperar respuesta síncrona con los ítems.
...
> **Bot conversacional — guía en vez de cortar (no responde "No tengo ese dato" seco).** Trae el patrón de clarificación de `alo-ai-engine` (ver [[reference_alo-engine-conversational-patterns]]). Detectores **regex-first** (sin LLM) en `telegram/searchIntent.ts`: `isBareSearchIntent` ("buscar a una persona" sin nombre) → pide el nombre y guarda **`pendingSearch`** en `MenuState` (DynamoDB, expira **30 min**); el siguiente turno se resuelve con `answerPersonSearch` (búsqueda determinista por título + aviso de localización), sin router. `bareCategoryAction` ("acopios"/"refugios" sin zona) → `locationPrompt` + `pendingCategory` (engancha el flujo de ubicación del menú). `isHelpCry` ("necesito ayuda"/"auxilio") → `helpScreen()` con botones (emergencias/refugios/víveres). El fallback `NO_DATA` ahora **orienta** sobre qué sí puede responder. Solo nombres de categoría inequívocos disparan `bareCategoryAction` ("dónde hay agua" sigue yendo a búsqueda). <!-- /aprende 2026-07-01 -->
